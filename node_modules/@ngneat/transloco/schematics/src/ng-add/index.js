...
const ng_ast_utils_1 = require("../utils/ng-schematics-utils/ng-ast-utils");
const util_1 = require("../utils/ng-schematics-utils/standalone/util");
const standalone_1 = require("../utils/ng-schematics-utils/standalone");
const app_config_1 = require("../utils/ng-schematics-utils/standalone/app_config");
...
function default_1(options) {
    return async (host, context) => {
        ...
        const modulePath = isStandalone
            ? ''
            : _path.dirname((0, ng_ast_utils_1.getAppModulePath)(host, mainPath));
        const actions = [];
        if (options.loader === schema_1.Loaders.Http) {
            ...
            if (isStandalone) {
                actions.push((0, standalone_1.addRootProvider)(options.project, ({ code, external }) => code `${external('provideHttpClient', '@angular/common/http')}()`));
            }
            else {
                actions.push((0, standalone_1.addRootImport)(options.project, ({ code, external }) => code `${external('HttpClientModule', '@angular/common/http')}`));
            }
        ...
        if (isStandalone) {
            actions.push((0, standalone_1.addRootProvider)(options.project, ({ code, external }) => {
        ...
        loader: TranslocoHttpLoader
            ...
            actions.push((0, standalone_1.addRootImport)(options.project, ({ code, external }) => code `${external('TranslocoRootModule', './transloco-root.module')}`), (0, schematics_1.mergeWith)((0, root_module_gen_1.createTranslocoModule)({
                sourceRoot,
                isLib,
                ssr: options.ssr,
                langs,
